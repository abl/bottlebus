...
class OneBusAway:
    ...
    def arrivalsAndDeparturesForStop(self, stop):
        ...
        for x in arrivals:
            ...
            x['minutesToScheduledArrival'] = ((x['scheduledArrivalTime'] - ctime)/6000+5)/10
        return sorted([(x['routeShortName'], x['minutesToArrival'], x['minutesToScheduledArrival']) for x in arrivals], key=lambda x:x[1])
